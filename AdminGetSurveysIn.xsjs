...
//var proced = "select count(*) as numberParticipated from \"  sp inner join \"Users.Users\" u on sp.\"Username\"=u.\"Username\" where ";
var procedureCa = "select count(distinct(sud.\"I sud inner join \\" sa on sud.\"Definition\"=sa.\"rDefinition\" where \ and \"IsTest\"=0";

//var procedureCallStatementSelectedParticipants = "select count(*) as numberParticipated froS\".electedParticipants\" sp inner join \"Users.Users\" u on sp.\"Username\"=u.\"Username\" where "=?";
...
var CallStatementGetInvitationsSent = 'select count("Username") "InvitationsSent" froicationsUsers" snu left joifications" sn on sfications" = snu."ificationsType" = 1 and "NotificationSent" = 1 and "IsSampleMail" = 0';

var deleteFrom=function(){
	var connection = $.db.getConnection($.services.utils.Utils.ConnectionName);  
	connection.prepareStatement("SET SCHEMA "  + $..services.utils..SchemaName).execute();
	var statement = null;
	var pom="empty";
	try{
		statement = connection.prepareStatement(deleteFromClonedTable);
...
var GetNumberParticipated=function(){	
	var connection = $.db.getConnection($..services.utils..ConnectionName);  
	connection.prepareStatement("SET SCHEMA "  + $..services.utils..SchemaName).execute();  
	var statement = null,resultSet = null,resultObject={},resultList=[];
	try{			
		
		statement = connection.prepareStatement(procedureCallStatemarticipants);
		statement.setInteger(1, parseInt(,0));
		resultSet = statement.executeQuery();
		while (resultSet.next()) {
			resultObject={};
...
var isInvitationMailChecked = function(){
	var connection = $.db.getConnection($..services.utils..ConnectionName);  
	connection.prepareStatement("SET SCHEMA "  + $..services.utils..SchemaName).execute();  
	var statement = null,resultSet = null,resultObject={},resultList=[];
	try{			
...
var GetInvitationsSent = function(){
	var connection = $.db.getConnection($..services.utils..ConnectionName);  
	connection.prepareStatement("SET SCHEMA "  + $..services.utils..SchemaName).execute();  
	var statement = null,resultSet = null,resultObject={},resultList=[];
	try{			
